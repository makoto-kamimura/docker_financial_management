# カケイカイケイ（financial-management / 決算管理システム）

> **カケイカイケイ** は本システムの呼称です（旧称: 決算管理システム）。
> 家計の「カケイ」と会計の「カイケイ」を掛け合わせ、家計管理から個人事業主・法人の会計・決算までを 1 つのアプリで扱うことを表しています。
> リポジトリ名・パッケージ名・Docker サービス名などの識別子は `financial-management` のまま変更していません。
