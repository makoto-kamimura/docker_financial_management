...
| GET/POST | `/api/transfers` | 口座間の振込/引き落としルールの取得・登録 |
| GET | `/api/transfers/flow` | 口座間の資金移動フロー図（Sankey）を自動生成 |
| POST | `/api/transfers/simulate` | 期首残高＋振込/引き落としから残高推移をシミュレートし残高不足を検出 |
...
- `/simulation` … 残高シミュレーション。期首残高と振込/引き落としから日次の残高推移を予測し、残高不足日を検出
...
- `transfers`（資金移動）: id, from_account_id, to_account_id, amount, kind(MANUAL/AUTO), day, note
