...
| GET | `/api/financials` | 実績データの集計結果（月次）を取得 |
| POST | `/api/financials` | 実績データの登録（Zod バリデーション） |
| GET | `/api/forecasts?months=N` | 過去実績から将来 N か月の推移を予測 |

> 現状はインメモリのサンプル実装。Phase 1 で Prisma + PostgreSQL に置き換える。
