...
export function resolvePeriodForDate(db: TenantDbClient, tenantId: number, date: Date) {
  return resolvePeriod(db, tenantId, date.getFullYear(), date.getMonth() + 1);
}

// 勘定科目をコードで解決する（自テナントのみ）。
export function findAccountByCode(db: TenantDbClient, tenantId: number, code: string) {
  return db.account.findUnique({ where: { tenantId_code: { tenantId, code } } });
}

// findAccountByCode の必須版。見つからなければ 400 を throw する。
export async function requireAccountByCode(db: TenantDbClient, tenantId: number, code: string) {
