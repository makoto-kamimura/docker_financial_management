...
type Totals = {
  ...
  expense: number; operatingProfit: number;
};
type Labels = {
  revenue: string; cogs: string; grossProfit: string;
  expense: string; operatingProfit: string;
};
...
function BarLabel({
  ...
  line1: string; line2: string; minW?: number;
}) {
  if (w < minW) return null;
  return (
    <>
      <text x={x + w / 2} y={y + h / 2 - 7} textAnchor="middle" fontSize={11} fill="white" fontWeight="700">
        {line1}
      </text>
      <text x={x + w / 2} y={y + h / 2 + 9} textAnchor="middle" fontSize={10} fill="rgba(255,255,255,0.85)">
        {line2}
      </text>
    </>
  );
...
function OuterLabel({
  ...
  return (
    <>
      <text x={x + w + 8} y={y + h / 2 - 6} textAnchor="start" fontSize={11} fill="#1e293b" fontWeight="600">
        {line1}
      </text>
  ...
  return (
    <div>
      <div className="overflow-x-auto">
        ...
        >
          ...
          <text
            ...
            fontSize={12} fill="#374151" fontWeight="700"
          >
            {lbl.revenue}
          </text>
          <rect x={bx} y={y0} width={BAR_MAX} height={BH} fill={C.rev} rx={6} />
          <BarLabel x={bx} y={y0} w={BAR_MAX} h={BH}
            line1={yen(rev)} line2="100%" minW={100} />
          ...
          {wCogs > 0 && (
            <g>
              ...
              <BarLabel
                x={bx + wGros} y={yCogs} w={wCogs} h={COST_H}
                line1={lbl.cogs}
          ...
          {wGros > 0 ? (
            <g>
              ...
              <BarLabel x={bx} y={y1} w={wGros} h={BH}
                line1={yen(totals.grossProfit)}
                line2={pct(totals.grossProfit, rev)} minW={90} />
              <OuterLabel x={bx} y={y1} w={wGros} h={BH}
                line1={yen(totals.grossProfit)}
                line2={pct(totals.grossProfit, rev)} minW={90} />
            </g>
          ) : (
            <text x={bx + 10} y={y1 + BH / 2} dominantBaseline="middle"
              fontSize={12} fill={C.loss} fontWeight="600">
          ...
          {wExp > 0 && wGros > 0 && (
            <g>
              ...
              <BarLabel
                x={bx + wProf} y={yExp} w={wExp} h={COST_H}
                line1={lbl.expense}
          ...
          {wProf > 0 ? (
            <g>
              ...
              <BarLabel x={bx} y={y2} w={wProf} h={BH}
                line1={yen(totals.operatingProfit)}
                line2={pct(totals.operatingProfit, rev)} minW={90} />
              <OuterLabel x={bx} y={y2} w={wProf} h={BH}
                line1={yen(totals.operatingProfit)}
                line2={pct(totals.operatingProfit, rev)} minW={90} />
            </g>
          ) : (
            <text x={bx + 10} y={y2 + BH / 2} dominantBaseline="middle"
              fontSize={12} fill={C.loss} fontWeight="600">
      ...
      <div className="flex flex-wrap gap-x-5 gap-y-1.5 mt-3 px-1">
        ...
        ] as [string, string, string][]).map(([color, label, amount]) => (
