...
export default function InvoicesPage() {
  ...
  const addLine = () =>
    setForm((f) => ({
      ...f,
      lines: [...f.lines, { description: "", quantity: "1", unitPrice: "", taxRate: "0.10" }],
    }));
  ...
  const setLine = (i: number, k: string, v: string) =>
    setForm((f) => ({
      ...f,
      lines: f.lines.map((l, j) => (j === i ? { ...l, [k]: v } : l)),
    }));
  ...
  return (
    <AppShell>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ["下書き", totalDraft, "text-slate-600"],
          ["発行済み", totalIssued, "text-blue-600"],
          ["入金済み", totalPaid, "text-green-600"],
        ].map(([l, v, c]) => (
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-xs text-slate-500">
              <tr>
                {["請求番号", "取引先", "発行日", "支払期限", "金額（税込）", "状態", "操作"].map(
                  (h) => (
              ...
              {invoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${STATUS[inv.status]?.color}`}
                    >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => window.open(`/invoices/print?id=${inv.id}`, "_blank")}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 overflow-y-auto py-8">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-2xl mx-4">
            ...
            <button onClick={addLine} className="text-sm text-indigo-600 hover:underline mb-4">
              + 行追加
            </button>
            <div className="mb-3">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 overflow-y-auto py-8">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg mx-4">
            ...
            <div className="text-sm space-y-1 mb-4">
              <p>
                <span className="text-slate-500">番号:</span> {preview.invoiceNumber}
              </p>
              <p>
                <span className="text-slate-500">取引先:</span> {preview.customerName}
              </p>
              {preview.customerAddress && (
                <p>
                  <span className="text-slate-500">住所:</span> {preview.customerAddress}
                </p>
              )}
              <p>
                <span className="text-slate-500">発行日:</span> {preview.issueDate.slice(0, 10)} /
                支払期限: {preview.dueDate.slice(0, 10)}
              </p>
            </div>
            <table className="w-full text-sm mb-4">
              <thead className="text-xs text-slate-500 border-b">
                <tr>
                  <th className="text-left pb-2">品目</th>
                  <th className="text-right pb-2">数量</th>
                  <th className="text-right pb-2">単価</th>
                  <th className="text-right pb-2">税率</th>
