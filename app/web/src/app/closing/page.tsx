...
function PnlTab({ pnl, sysMode }: { pnl: Statements["pnl"]; sysMode: ViewMode }) {
  return (
    <div className="space-y-4">
        ...
        <table className="w-full text-sm">
          <tbody className="divide-y divide-slate-50">
            ...
            <tr className="font-semibold border-t border-slate-200">
              ...
              <td className="py-2 text-right text-emerald-700 text-base">
                {yen(pnl.revenueTotal)}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <section>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-50">
                ...
                {pnl.expenses.some((e) => (e.businessRate ?? 100) < 100) && (
                  <>
                    <td className="py-1.5 text-right text-xs text-slate-400">
                      {a.businessRate === 100 || a.businessRate === undefined
                        ? "—"
                        : `${a.businessRate}%`}
                    </td>
                    <td className="py-1.5 text-right font-medium text-red-600">
                      {yen(a.deductible ?? a.total)}
                    </td>
                  </>
                )}
              </tr>
            ...
            <tr className="font-semibold border-t border-slate-200">
              <td
                colSpan={pnl.expenses.some((e) => (e.businessRate ?? 100) < 100) ? 4 : 2}
      ...
      {/* 事業所得 */}
      <div
        className={`border rounded-lg px-4 py-4 flex justify-between items-center ${
          pnl.netIncome >= 0 ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"
        }`}
      >
        <div>
          <p
            className={`font-bold text-lg ${pnl.netIncome >= 0 ? "text-emerald-800" : "text-red-700"}`}
          >
            事業所得（課税所得）
          </p>
        ...
        <span
          className={`text-2xl font-bold ${pnl.netIncome >= 0 ? "text-emerald-700" : "text-red-600"}`}
        >
          {yen(pnl.netIncome)}
        </span>
      </div>
        ...
        <table className="w-full text-sm">
          <tbody className="divide-y divide-slate-50">
            ...
            <tr className="font-semibold">
              ...
              <td
                className={`py-2 text-right font-bold ${bs.equity >= 0 ? "text-indigo-700" : "text-red-600"}`}
              >
                {yen(bs.equity)}
              </td>
            </tr>
            ...
            <tr key={a.accountId} className="hover:bg-slate-50/60">
              ...
              <td
                className={`py-1.5 text-right font-medium ${
                  ["ASSET", "EXPENSE", "COGS"].includes(a.category)
                    ? "text-slate-800"
                    : "text-emerald-700"
                }`}
              >
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-5">
      ...
      <div className="flex items-baseline gap-1.5">
        {value === null ? (
          ...
          <>
            <span
              className={`text-2xl font-bold ${good(value) ? "text-indigo-700" : "text-red-600"}`}
            >
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-50 rounded-xl p-5">
        ...
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div>
            <p className="text-slate-500 text-xs mb-0.5">総資産</p>
            <p className="font-semibold">{yen(bs.assetTotal)}</p>
          </div>
          <div>
            <p className="text-slate-500 text-xs mb-0.5">総負債</p>
            <p className="font-semibold">{yen(bs.liabilityTotal)}</p>
          </div>
          <div>
            <p className="text-slate-500 text-xs mb-0.5">純資産</p>
            <p className="font-semibold">{yen(bs.equity)}</p>
          </div>
          <div>
            <p className="text-slate-500 text-xs mb-0.5">売上高</p>
            <p className="font-semibold">{yen(pnl.revenueTotal)}</p>
          </div>
          <div>
            <p className="text-slate-500 text-xs mb-0.5">売上総利益</p>
            <p className="font-semibold">{yen(pnl.grossProfit)}</p>
          </div>
          <div>
            <p className="text-slate-500 text-xs mb-0.5">当期純利益</p>
            <p className="font-semibold">{yen(pnl.netIncome)}</p>
          </div>
        </div>
      </div>
    </div>
          ...
          {months.map((m) => {
            ...
            return (
              <tr key={m} className="hover:bg-slate-50/60">
                ...
                <td
                  className={`py-1.5 text-right font-medium ${profit >= 0 ? "text-slate-800" : "text-red-600"}`}
                >
                  {yen(profit)}
                </td>
              </tr>
          ...
          <tr className="font-semibold border-t-2 border-slate-300">
            ...
            <td
              className={`py-2 text-right ${
                totals.revenue - totals.cogs - totals.expense >= 0
                  ? "text-indigo-700"
                  : "text-red-600"
              }`}
            >
...
export default function ClosingPage() {
  ...
  const { data, isLoading } = useQuery<Statements>({
    ...
    queryFn: () => fetch(`/api/closing/statements?year=${year}`).then((r) => r.json()),
  });
          ...
          {!isClosed ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => {
                  if (confirm("決算を取り消して再開放しますか？")) reopenMut.mutate(year);
                }}
        ...
        <>
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            ...
            <div className="card">
              ...
              <p
                className={`text-lg font-bold ${data.pnl.netIncome >= 0 ? "text-indigo-700" : "text-red-600"}`}
              >
