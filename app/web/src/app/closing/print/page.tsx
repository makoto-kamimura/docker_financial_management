...
type Statements = {
  ...
  businessProfile: {
    tradeName: string | null; ownerName: string;
    blueReturn: boolean; taxationType: string;
    invoiceNumber: string | null; openedOn: string | null;
  } | null;
  closeStatus: { status: string; closedAt: string | null } | null;
};

const yen = (v: number) =>
  Math.abs(v).toLocaleString("ja-JP");
      ...
      <div className="max-w-4xl mx-auto p-8">
        ...
        <div className="mb-10 text-center">
          ...
          <div className="mt-4 inline-block text-left border border-gray-300 rounded px-6 py-3">
            <table style={{ width: "auto", borderCollapse: "separate", border: "none" }}>
              <tbody>
                <tr>
                  <td style={{ border: "none", padding: "2px 12px 2px 0", color: "#666", fontSize: "10pt" }}>氏名</td>
                  <td style={{ border: "none", padding: "2px 0", fontWeight: "bold", fontSize: "11pt" }}>{ownerName}</td>
                </tr>
                {tradeName && (
                  <tr>
                    <td style={{ border: "none", padding: "2px 12px 2px 0", color: "#666", fontSize: "10pt" }}>屋号</td>
                    <td style={{ border: "none", padding: "2px 0", fontWeight: "bold", fontSize: "11pt" }}>{tradeName}</td>
                  </tr>
                )}
                {invoiceNo && (
                  <tr>
                    <td style={{ border: "none", padding: "2px 12px 2px 0", color: "#666", fontSize: "10pt" }}>登録番号</td>
                    <td style={{ border: "none", padding: "2px 0", fontSize: "10pt" }}>{invoiceNo}</td>
                  </tr>
                )}
                <tr>
                  <td style={{ border: "none", padding: "2px 12px 2px 0", color: "#666", fontSize: "10pt" }}>作成日</td>
                  <td style={{ border: "none", padding: "2px 0", fontSize: "10pt" }}>{today}</td>
                </tr>
                {closeStatus?.closedAt && (
                  <tr>
                    <td style={{ border: "none", padding: "2px 12px 2px 0", color: "#666", fontSize: "10pt" }}>決算確定日</td>
                    <td style={{ border: "none", padding: "2px 0", fontSize: "10pt", color: "#059669" }}>
        ...
        <div className="doc-section">
          ...
          <table>
            ...
            <tbody>
              {/* 収入 */}
              <tr><td colSpan={3} style={{ background: "#dbe4ff", fontWeight: "bold", padding: "4px 6px" }}>【収入金額】</td></tr>
              {pnl.revenue.map(a => (
                <tr key={a.accountId}>
              ...
              <tr className="total-row">
                <td colSpan={2} style={{ textAlign: "right" }}>収入合計</td>
              ...
              {pnl.cogs.length > 0 && (
                <>
                  <tr><td colSpan={3} style={{ background: "#dbe4ff", fontWeight: "bold", padding: "4px 6px" }}>【売上原価】</td></tr>
                  {pnl.cogs.map(a => (
                    <tr key={a.accountId}>
                      <td style={{ textAlign: "center", fontSize: "9pt" }}>{a.code}</td>
                      <td>{a.name}</td>
                  ...
                  <tr className="total-row">
                    <td colSpan={2} style={{ textAlign: "right" }}>原価合計</td>
              ...
              <tr style={{ background: "#e0f2fe", fontWeight: "bold" }}>
                <td colSpan={2} style={{ textAlign: "right" }}>売上総利益</td>
                <td className="amount">{yen(pnl.grossProfit)}</td>
              </tr>

              {/* 経費 */}
              <tr><td colSpan={3} style={{ background: "#dbe4ff", fontWeight: "bold", padding: "4px 6px" }}>
                【経費（必要経費）】
              </td></tr>
              {pnl.expenses.map(a => (
                <tr key={a.accountId}>
          ...
          <table>
            ...
            <tbody>
              ...
              <tr className="total-row">
                ...
                <td className="amount">{yen(pnl.revenueTotal - pnl.cogsTotal - pnl.expenseDeductible)}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        {isBlue && (
          <div className="doc-section page-break">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              <div>
                <table>
                  <thead><tr><th colSpan={2}>資産の部</th></tr></thead>
                  <tbody>
                    {bs.assets.map(a => (
                      <tr key={a.accountId}>
                        <td>{a.name}<span style={{ fontSize: "8pt", color: "#666", marginLeft: "4px" }}>{a.code}</span></td>
                        <td className="amount">{yen(a.total)}</td>
                      </tr>
                    ))}
                    <tr className="total-row">
              ...
              <div>
                <table>
                  <thead><tr><th colSpan={2}>負債・資本の部</th></tr></thead>
                  <tbody>
        ...
        {pnl.expenses.some(e => (e.businessRate ?? 100) < 100) && (
          <div className="doc-section">
            <h2 className="text-lg font-bold border-b-2 border-gray-800 mb-3 pb-1">家事按分計算書</h2>
            <table>
              <thead>
                <tr>
                  <th>科目名</th>
              ...
              <tbody>
                {pnl.expenses.filter(e => (e.businessRate ?? 100) < 100).map(e => (
                  <tr key={e.accountId}>
                    <td>{e.name}</td>
                ...
                <tr className="total-row">
                  <td colSpan={3} style={{ textAlign: "right" }}>按分後経費合計</td>
                  <td className="amount">{yen(pnl.expenseDeductible)}</td>
        ...
        <p style={{ marginTop: "2rem", fontSize: "8pt", color: "#999", textAlign: "right" }}>
          出力日時: {new Date().toLocaleString("ja-JP")} ／ {isBlue ? "青色申告" : "白色申告（収支内訳書）"}
        </p>
      </div>
    </>
  );
