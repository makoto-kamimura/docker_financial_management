...
export async function POST(req: NextRequest) {
  ...
  if (!account) return NextResponse.json({ error: `勘定科目 "${body.accountCode}" が見つかりません` }, { status: 400 });
  if (!counter) return NextResponse.json({ error: `対当科目 "${body.counterAccountCode}" が見つかりません` }, { status: 400 });

  const [debitId, creditId] = body.direction === "income"
    ? [counter.id, account.id]
    : [account.id, counter.id];
