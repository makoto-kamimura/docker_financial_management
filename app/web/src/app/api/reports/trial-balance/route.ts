  ...
  for (const d of details) {
    ...
    if (!map.has(a.id)) {
      map.set(a.id, {
        ...
        parentId: a.parentId, depth: 0,
        totalDebit: 0, totalCredit: 0, balance: 0, isGroup: false,
      });
    }
  ...
  if (rollup) {
    ...
    for (const [, row] of map) {
      ...
      while (parentId != null) {
        ...
        if (!map.has(parentId)) {
          map.set(parentId, {
            id: parentId, code: parent.code, name: parent.name, category: parent.category,
            parentId: parent.parentId, depth: 0,
            totalDebit: 0, totalCredit: 0, balance: 0, isGroup: true,
          });
        }
