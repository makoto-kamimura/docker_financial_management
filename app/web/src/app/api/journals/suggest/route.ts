...
export async function POST(req: NextRequest) {
  ...
  // 1. 過去仕訳の摘要テキストを部分一致で検索（最大5件）
  const pastEntries = await prisma.journalEntry.findMany({
    where: { description: { contains: body.description, mode: "insensitive" } },
    include: {
      details: { include: { account: { select: { id: true, code: true, name: true, category: true } } } },
    },
  ...
  if (pastEntries.length > 0) {
    ...
    return NextResponse.json({
      data: {
        ...
        debit:  debitLines.map(d => ({ accountId: d.accountId, accountCode: d.account.code, accountName: d.account.name, amount: Number(d.amount) })),
        credit: creditLines.map(d => ({ accountId: d.accountId, accountCode: d.account.code, accountName: d.account.name, amount: Number(d.amount) })),
      },
    });
  }
  ...
  for (const rule of rules) {
    if (rule.keywords.some(k => q.includes(k))) {
      const [debitAcc, creditAcc] = await Promise.all([
        prisma.account.findFirst({ where: { code: rule.debitCode } }),
        prisma.account.findFirst({ where: { code: rule.creditCode } }),
      ]);
