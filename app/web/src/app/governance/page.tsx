...
export default function GovernancePage() {
  ...
  const [officerForm, setOF] = useState({
    tenantId: "",
    name: "",
    title: "",
    termStart: "",
  ...
  const [meetingForm, setMF] = useState({
    tenantId: "",
    meetingDate: "",
    meetingType: "regular",
    agenda: "",
  ...
  const saveOfficer = async () => {
    const r = await fetch("/api/officers", {
      ...
      body: JSON.stringify({
        ...officerForm,
        tenantId: Number(officerForm.tenantId),
        salary: officerForm.salary ? Number(officerForm.salary) : undefined,
      }),
    });
    if (r.ok) {
      setShowForm(false);
  ...
  const saveMeeting = async () => {
    const r = await fetch("/api/shareholder-meetings", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const saveDiv = async () => {
    const r = await fetch("/api/dividends", {
      ...
      body: JSON.stringify({
        ...divForm,
        tenantId: Number(divForm.tenantId),
        perShareAmount: Number(divForm.perShareAmount),
        totalAmount: Number(divForm.totalAmount),
      }),
    });
    if (r.ok) {
      setShowForm(false);
  ...
  const saveAnn = async () => {
    const r = await fetch("/api/announcements", {
      ...
      body: JSON.stringify({
        ...annForm,
        tenantId: Number(annForm.tenantId),
        fiscalYear: Number(annForm.fiscalYear),
      }),
    });
    if (r.ok) {
      setShowForm(false);
  ...
  const delOfficer = async (id: number) => {
  ...
  const delMeeting = async (id: number) => {
    if (!confirm("削除しますか？")) return;
    await fetch(`/api/shareholder-meetings/${id}`, { method: "DELETE" });
    loadAll();
  };
        ...
        <>
          {tab === "officers" && (
            <div className="space-y-3">
              {officers.length === 0 ? (
                <p className="text-slate-400 text-center py-12">役員情報がありません。</p>
              ) : (
                officers.map((o) => (
                  <div
                    key={o.id}
                  ...
                  >
                    <div>
                      ...
                      <div className="text-sm text-slate-500 mt-0.5">
                        ...
                        {o.salary && (
                          <span className="ml-4">
                            報酬: ¥{Number(o.salary).toLocaleString()}/月
                          </span>
                        )}
                      </div>
                    </div>
                    <button
                      onClick={() => delOfficer(o.id)}
          ...
          {tab === "meetings" && (
            <div className="space-y-3">
              {meetings.length === 0 ? (
                <p className="text-slate-400 text-center py-12">株主総会の記録がありません。</p>
              ) : (
                meetings.map((m) => (
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full ${m.meetingType === "regular" ? "bg-blue-100 text-blue-700" : "bg-orange-100 text-orange-700"}`}
                          >
                        ...
                        {m.resolution && (
                          <p className="text-sm text-slate-500 mt-1">決議: {m.resolution}</p>
                        )}
                      </div>
                      <button
                        onClick={() => delMeeting(m.id)}
                        className="text-sm text-red-500 hover:underline"
                      >
                        削除
                      </button>
          ...
          {tab === "announcements" && (
            <div className="space-y-3">
              {announcements.length === 0 ? (
                <p className="text-slate-400 text-center py-12">決算公告の記録がありません。</p>
              ) : (
                announcements.map((a) => (
                  <div
                    key={a.id}
            ...
            <div className="space-y-3">
              <div>
                ...
                <select
                  ...
                  value={
                    tab === "officers"
                      ? officerForm.tenantId
                      : tab === "meetings"
                        ? meetingForm.tenantId
                        : tab === "dividends"
                          ? divForm.tenantId
                          : annForm.tenantId
                  }
                  ...
                  {[
                    ...
                    <div key={k}>
                      ...
                      <input
                        ...
                        value={officerForm[k as keyof typeof officerForm]}
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setMF((f) => ({ ...f, meetingDate: e.target.value }))}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setMF((f) => ({ ...f, meetingType: e.target.value }))}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                    >
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setMF((f) => ({ ...f, agenda: e.target.value }))}
                      rows={2}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      value={meetingForm.resolution}
                      onChange={(e) => setMF((f) => ({ ...f, resolution: e.target.value }))}
                      rows={2}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                    />
                  </div>
              ...
              {tab === "dividends" && (
                <>
                  {[
                    ["resolutionDate", "決議日 *", "date"],
                    ["paymentDate", "支払日 *", "date"],
                    ["perShareAmount", "1株配当額（円）"],
                    ["totalAmount", "配当総額（円）*"],
                  ].map(([k, l, type]) => (
                    <div key={k}>
                      <label className="block text-sm font-medium text-slate-600 mb-1">{l}</label>
                      <input
                        type={type ?? "number"}
                        value={divForm[k as keyof typeof divForm]}
              ...
              {tab === "announcements" && (
                <>
                  <div>
                    ...
                    <input
                      type="date"
                      value={annForm.announcementDate}
                      onChange={(e) => setAF((f) => ({ ...f, announcementDate: e.target.value }))}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      value={annForm.method}
                      onChange={(e) => setAF((f) => ({ ...f, method: e.target.value }))}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                    >
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={annForm.fiscalYear}
                      onChange={(e) => setAF((f) => ({ ...f, fiscalYear: e.target.value }))}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setAF((f) => ({ ...f, content: e.target.value }))}
                      rows={3}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                    />
                  </div>
