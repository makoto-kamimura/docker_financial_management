...
function ApportionmentForm({
  ...
  const expenseAccounts = accounts.filter((a) => ["EXPENSE", "COGS"].includes(a.category));
...
export default function AppportionmentsPage() {
  ...
  const { data: list = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      fetch("/api/apportionments")
        .then((r) => r.json())
        .then((r) => r.data as Apportionment[]),
  });
      ...
      {isLoading ? (
        ...
        <div className="card text-center py-12 text-slate-400">
          ...
          <p className="text-xs mt-1">
            自宅兼事務所の家賃・光熱費など、家事按分が必要な費用科目を設定してください。
          </p>
        </div>
      ) : (
                    ...
                    <td className="py-2.5">
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            if (confirm("削除しますか？")) delMut.mutate(a.accountId);
                          }}
