...
export default function JournalTemplatesPage() {
  ...
  const addLine = () =>
    setForm((f) => ({
      ...f,
      lines: [...f.lines, { side: "debit", accountId: "", amount: "", note: "" }],
    }));
          ...
          {templates.map((t) => (
            <div key={t.id} className="bg-white rounded-xl border border-slate-200 p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <button onClick={() => del(t.id)} className="text-sm text-red-500 hover:underline">
                  削除
                </button>
              </div>
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-50">
                  {t.lines.map((l, i) => (
                    <tr key={i}>
                      <td
                        className={`py-1 font-medium text-xs ${l.side === "debit" ? "text-blue-600" : "text-red-600"}`}
                      >
                        {l.side === "debit" ? "借方" : "貸方"}
                      </td>
                      <td className="py-1">
            ...
            <div className="space-y-2 mb-3">
              {form.lines.map((l, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-center">
                  <select
                    value={l.side}
                    onChange={(e) => setLine(i, "side", e.target.value)}
