...
type Txn = { id: number; date: string; description: string; amount: number; balance: number | null; source: "MANUAL" | "CSV" | "SYNC" };
type Transfer = {
  id: number; label: string | null; day: number; amount: number;
  channel: string; kind: string; note: string | null;
  fromAccountId: number | null; toAccountId: number | null;
  fromAccount: BankAccount | null; toAccount: BankAccount | null;
};
type ImportResult = { inserted: number; errors: { row: number; message: string }[] };
type TransferFlowResponse = {
  cyclic: boolean;
  graph: FlowGraph;
  transfers: { id: number; from: string | null; to: string | null; amount: number; channelLabel: string; label: string | null; day: number }[];
};
...
const CHANNEL_LABELS: Record<string, string> = {
  INCOME: "給与・収入", EXPENSE: "支出", BANK_TRANSFER: "銀行振込",
  AUTO_DEBIT: "自動引落し", CARD_PAYMENT: "カード引落し",
};
const SOURCE_LABELS: Record<string, string> = { MANUAL: "手動", CSV: "CSV", SYNC: "自動取得" };

const BLANK_MANUAL    = { date: now.toISOString().slice(0, 10), description: "", amount: "", balance: "", type: "expense" as "income" | "expense" };
const BLANK_RECURRING = { label: "", channel: "AUTO_DEBIT" as string, day: 25, amount: "", note: "", direction: "out" as "in" | "out" };
...
export default function BankTransactionsPage() {
  ...
  const transfers = useMemo(
    () => (allTransfers ?? []).filter(t => t.fromAccountId === accountId || t.toAccountId === accountId),
  ...
  async function deleteTxn(txnId: number) {
    await fetch(`/api/bank-accounts/${accountId}/transactions?txnId=${txnId}`, { method: "DELETE" });
    qc.invalidateQueries({ queryKey: ["bank-txns", accountId] });
  }
  ...
  async function submitManual(e: { preventDefault(): void }) {
    ...
    const res = await fetch(`/api/bank-accounts/${accountId}/transactions`, {
      ...
      body: JSON.stringify({ date: manual.date, description: manual.description, amount, balance: manual.balance ? Number(manual.balance) : null }),
    });
    if (res.ok) { setManual(BLANK_MANUAL); setMsg("登録しました"); qc.invalidateQueries({ queryKey: ["bank-txns", accountId] }); }
    ...
    <AppShell>
        ...
        {TABS.map(([t, label]) => (
          ...
          >
            ...
            {t === "recurring" && transfers.length > 0 && (
              <span className="ml-1.5 text-xs bg-slate-200 text-slate-600 rounded-full px-1.5">{transfers.length}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {msg && (
        <div className="mb-4 text-sm text-slate-700 bg-indigo-50 border border-indigo-100 rounded-md px-3 py-2 flex items-center justify-between">
          {msg}
          <button onClick={() => setMsg(null)} className="text-slate-400 hover:text-slate-600 ml-4">✕</button>
        </div>
      )}
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="text-left px-3 py-2 font-semibold text-slate-600 hidden sm:table-cell">→</th>
                    ...
                    <th className="text-left px-3 py-2 font-semibold text-slate-600 hidden md:table-cell">方式</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {flowData.transfers.map(t => (
                    <tr key={t.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-3 py-2 text-slate-700">{t.from ?? <span className="text-emerald-600 font-medium">{t.label ?? "外部入金"}</span>}</td>
                      <td className="px-3 py-2 text-slate-400 hidden sm:table-cell">→</td>
                      <td className="px-3 py-2 text-slate-700">{t.to ?? <span className="text-rose-600 font-medium">{t.label ?? "外部支出"}</span>}</td>
                      <td className="px-3 py-2 text-right font-medium tabular-nums text-slate-900">{yen(t.amount)}</td>
                      <td className="px-3 py-2 text-slate-400 text-xs hidden md:table-cell">{t.channelLabel}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="card mb-4">
            ...
            <form onSubmit={submitManual} className="flex flex-wrap gap-3 items-end">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input type="date" required value={manual.date} onChange={e => setManual(m => ({ ...m, date: e.target.value }))} className="input-field text-sm" />
              </div>
              <div className="flex flex-col gap-1 min-w-40">
                <label className="text-xs text-slate-500">摘要</label>
                <input type="text" required placeholder="例: 食料品" value={manual.description} onChange={e => setManual(m => ({ ...m, description: e.target.value }))} className="input-field text-sm" />
              </div>
              <div className="flex flex-col gap-1 w-36">
                <label className="text-xs text-slate-500">金額（円）</label>
                <input type="number" required min={1} placeholder="例: 5000" value={manual.amount} onChange={e => setManual(m => ({ ...m, amount: e.target.value }))} className="input-field text-sm" />
              </div>
              <div className="flex flex-col gap-1 w-36">
                <label className="text-xs text-slate-500">残高（任意）</label>
                <input type="number" placeholder="例: 320000" value={manual.balance} onChange={e => setManual(m => ({ ...m, balance: e.target.value }))} className="input-field text-sm" />
              </div>
              <button type="submit" className="btn-primary px-5 py-2 text-sm self-end">登録する</button>
            </form>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {(txns ?? []).map(t => (
                  <tr key={t.id} className="hover:bg-slate-50 group">
                    <td className="px-4 py-2.5 whitespace-nowrap text-slate-500">{new Date(t.date).toLocaleDateString("ja-JP")}</td>
                    <td className="px-4 py-2.5">{t.description}</td>
                    <td className={`px-4 py-2.5 text-right tabular-nums ${t.amount < 0 ? "text-red-600" : "text-emerald-600"}`}>{yen(t.amount)}</td>
                    <td className="px-4 py-2.5 text-right tabular-nums text-slate-400">{t.balance != null ? yen(t.balance) : "—"}</td>
                    <td className="px-4 py-2.5 text-xs text-slate-400">{SOURCE_LABELS[t.source] ?? t.source}</td>
                    <td className="px-2 py-2.5 text-right">
                      <button onClick={() => deleteTxn(t.id)} className="opacity-0 group-hover:opacity-100 text-xs text-slate-300 hover:text-red-500 transition-opacity">削除</button>
                    </td>
                  </tr>
                ))}
                {(txns ?? []).length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400 text-sm">明細がありません</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {tab === "csv" && (
        <div className="max-w-2xl space-y-6">
          ...
          <div className="card flex items-center justify-between">
            ...
            <button onClick={sync} className="btn-secondary px-4 py-2 text-sm whitespace-nowrap">同期する</button>
          </div>
            ...
            <div className={`card border ${importResult.errors.length === 0 ? "border-green-200 bg-green-50" : "border-amber-200 bg-amber-50"}`}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  {importResult.errors.length > 0 && <p className="text-xs text-amber-700">{importResult.errors.length} 件のエラーがあります</p>}
                </div>
              </div>
              {importResult.errors.length > 0 && (
                <div className="mt-3 max-h-48 overflow-y-auto">
                  <table className="w-full text-xs">
                    <thead><tr className="text-slate-500 border-b border-amber-200"><th className="text-left pb-1 w-16">行番号</th><th className="text-left pb-1">エラー内容</th></tr></thead>
                    <tbody className="divide-y divide-amber-100">
                      {importResult.errors.map((err, i) => (
                        <tr key={i} className="text-slate-600"><td className="py-1 font-mono">{err.row}</td><td className="py-1">{err.message}</td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
...
2026-06-30,電気代,-8500,`}</pre>
            <ul className="mt-3 space-y-1 text-xs text-slate-500">
              <li><span className="font-medium">date</span>：取引日（YYYY-MM-DD）</li>
              <li><span className="font-medium">description</span>：摘要</li>
              <li><span className="font-medium">amount</span>：金額（収入は正、支出は負）</li>
              <li><span className="font-medium">balance</span>：取引後残高（任意）</li>
            </ul>
          </div>
        </div>
      )}
        ...
        <div className="flex gap-4 items-start">
          ...
          <div className="card flex-1 min-w-0 p-0 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
              <button onClick={prevCalMonth} className="p-1.5 rounded hover:bg-slate-100 text-slate-500">
                <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd" />
                </svg>
              </button>
              <span className="font-semibold text-slate-800">{viewYear}年{viewMonth}月</span>
              <button onClick={nextCalMonth} className="p-1.5 rounded hover:bg-slate-100 text-slate-500">
                <svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    {isValid && (
                      <>
                        ...
                        {dayTxs.slice(0, 2).map(t => {
                          ...
                          return (
                            <p key={t.id} className={`text-[10px] truncate leading-tight ${isOut ? "text-rose-600" : "text-emerald-600"}`}>
                              {isOut ? "−" : "+"}{t.label ?? CHANNEL_LABELS[t.channel] ?? t.channel}
                            </p>
                          );
                        })}
                        {dayTxs.length > 2 && (
            ...
            {selectedDay ? (
              <>
                ...
                <div className="card py-2 px-4">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{selectedDayTransfers.length} 件の固定入出金</p>
                </div>
                        ...
                        return (
                          <li key={t.id} className="flex items-center gap-2 px-3 py-2.5">
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-medium text-slate-800 truncate">{t.label ?? "—"}</p>
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                              <span className={`text-xs font-semibold ${isOut ? "text-rose-600" : "text-emerald-600"}`}>
                                {isOut ? "−" : "+"}{yen(Number(t.amount))}
                              </span>
                              <button
                ...
                {/* 追加フォーム */}
                <div className="card">
                  <h3 className="text-xs font-semibold text-slate-600 mb-3">毎月{selectedDay}日 の入出金を追加</h3>
                  <form onSubmit={submitRecurring} className="flex flex-col gap-2.5">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <input type="text" placeholder="例: 家賃・給与振込" value={recurring.label} onChange={e => setRecurring(r => ({ ...r, label: e.target.value }))} className="input-field text-xs" />
                    </div>
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] text-slate-500">種別</label>
                      <select value={recurring.channel} onChange={e => setRecurring(r => ({ ...r, channel: e.target.value }))} className="input-field text-xs">
                        {Object.entries(CHANNEL_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                      </select>
                    </div>
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] text-slate-500">金額（円）</label>
                      <input type="number" min={1} required placeholder="例: 90000" value={recurring.amount} onChange={e => setRecurring(r => ({ ...r, amount: e.target.value }))} className="input-field text-xs" />
                    </div>
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] text-slate-500">メモ（任意）</label>
                      <input type="text" placeholder="備考" value={recurring.note} onChange={e => setRecurring(r => ({ ...r, note: e.target.value }))} className="input-field text-xs" />
                    </div>
                    <button type="submit" className="btn-primary text-xs py-2 mt-1">追加</button>
                  </form>
                </div>
              </>
            ) : (
              <div className="card text-center py-8">
                <p className="text-sm text-slate-400">カレンダーの日付をクリックして<br />固定の入出金を確認・追加</p>
              </div>
            )}
          </div>
        </div>
