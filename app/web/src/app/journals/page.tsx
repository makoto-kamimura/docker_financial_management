...
type JournalDetail = {
  id: number; side: string; amount: number; note: string | null;
  account: Account;
};
...
const PAYMENT_LABELS: Record<string, string> = {
  cash: "現金", bank: "銀行振込", card: "クレジット", transfer: "口座引落",
};
const TAX_LABELS: Record<string, string> = {
  taxable: "課税", exempt: "非課税", non_taxable: "不課税",
};
...
function JournalForm({
  ...
  async function save() {
    ...
    const res = await fetch("/api/journals", {
      ...
      body: JSON.stringify({
        ...
        taxCategory:     tax,
        details: filled.map(d => ({ side: d.side, accountId: d.accountId, amount: d.amount, note: d.note || undefined })),
      }),
    });
  ...
  const acctOptions = (side: "debit" | "credit") => {
    const cats = side === "debit"
      ? ["EXPENSE", "COGS", "ASSET"]
      : ["REVENUE", "LIABILITY", "ASSET"];
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="block">
              ...
              <select className="input-field mt-1 w-full" value={method} onChange={e => setMethod(e.target.value)}>
          ...
          <label className="block">
            ...
            <input className="input-field mt-1 w-full" value={desc}
              onChange={e => setDesc(e.target.value)} placeholder="例: 4月分事務所家賃" />
          </label>
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="text-xs font-medium text-slate-600">税区分</span>
              <select className="input-field mt-1 w-full" value={tax} onChange={e => setTax(e.target.value)}>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {[...debitRows, ...creditRows].map(row => (
                  <tr key={row.idx} className={row.side === "debit" ? "bg-blue-50/30" : "bg-orange-50/30"}>
                    <td className="px-3 py-1.5">
                      <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${
                        row.side === "debit" ? "bg-blue-100 text-blue-700" : "bg-orange-100 text-orange-700"
                      }`}>
                    ...
                    <td className="px-3 py-1.5">
                      <input className="text-xs border border-slate-200 rounded px-1.5 py-1 w-full"
                        value={row.note}
                        onChange={e => updateDetail(row.idx, "note", e.target.value)} />
                    </td>
                    <td className="px-2 py-1.5 text-center">
                      {details.filter(d => d.side === row.side).length > 1 && (
                        <button onClick={() => removeRow(row.idx)}
          ...
          <div className="flex gap-2">
            <button onClick={() => addRow("debit")}
              className="text-xs text-blue-600 hover:underline">＋ 借方行を追加</button>
            <button onClick={() => addRow("credit")}
              className="text-xs text-orange-600 hover:underline">＋ 貸方行を追加</button>
          </div>

          {err && <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{err}</p>}
        </div>
        <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-2">
          <button onClick={onClose} className="btn-secondary text-sm px-4 py-1.5">キャンセル</button>
...
export default function JournalsPage() {
  ...
  useEffect(() => {
    fetch("/api/accounts")
      .then(r => r.json())
      .then(({ data }) => setAccounts((data as Account[]).filter(
        a => !["ASSET", "LIABILITY"].includes(a.category) || a.category === "ASSET" || a.category === "LIABILITY"
      )));
  }, []);
    ...
    <AppShell>
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="flex items-center gap-1.5">
          ...
          <select className="text-xs border border-slate-300 rounded-md px-2 py-1.5 bg-white"
                ...
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {(() => {
                      ...
                      return (
                        <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${ap.color}`}>{ap.label}</span>
                      );
                    })()}
                  ...
                  <table className="w-full text-xs">
                    <tbody className="divide-y divide-slate-50">
                      {[...debits, ...credits].map(d => (
                        <tr key={d.id} className={d.side === "debit" ? "bg-blue-50/20" : "bg-orange-50/20"}>
                          <td className="px-3 py-1.5 w-12">
                            <span className={`font-medium ${d.side === "debit" ? "text-blue-600" : "text-orange-600"}`}>
                              {d.side === "debit" ? "借方" : "貸方"}
                            </span>
                          </td>
                          ...
                          <td className="px-3 py-1.5 text-right font-medium">{yen(Number(d.amount))}</td>
