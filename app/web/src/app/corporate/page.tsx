...
type Tenant = {
  id: number; type: string; name: string; corporateNumber: string | null;
  capitalAmount: string | null; establishedOn: string | null; closingMonth: number;
};
...
export default function CorporatePage() {
  ...
  const [form, setForm] = useState({
    type: "CORPORATION", name: "", corporateNumber: "",
    capitalAmount: "", establishedOn: "", closingMonth: "12",
  });

  const load = () => {
    setLoading(true);
    fetch("/api/tenants").then(r => r.json()).then(j => {
      setTenants(j.data ?? []);
      setLoading(false);
    });
  };
  ...
  const openNew = () => {
    setEditing(null);
    setForm({ type: "CORPORATION", name: "", corporateNumber: "", capitalAmount: "", establishedOn: "", closingMonth: "12" });
    setShowForm(true);
  };
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${t.type === "CORPORATION" ? "bg-indigo-100 text-indigo-700" : "bg-green-100 text-green-700"}`}>
                      {typeLabel(t.type)}
                    </span>
                ...
                <div className="flex gap-2">
                  <button onClick={() => openEdit(t)} className="text-sm text-indigo-600 hover:underline">編集</button>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg">
            <h2 className="text-lg font-bold text-slate-800 mb-4">{editing ? "編集" : "新規登録"}</h2>
            <div className="space-y-3">
              ...
              <div>
                ...
                <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" placeholder="株式会社〇〇 / 〇〇商店" />
              </div>
              {form.type === "CORPORATION" && (
                <>
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1">法人番号（13桁）</label>
                    <input value={form.corporateNumber} onChange={e => setForm(f => ({ ...f, corporateNumber: e.target.value }))}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" placeholder="1234567890123" maxLength={13} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1">資本金（円）</label>
                    <input type="number" value={form.capitalAmount} onChange={e => setForm(f => ({ ...f, capitalAmount: e.target.value }))}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" placeholder="10000000" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1">設立日</label>
                    <input type="date" value={form.establishedOn} onChange={e => setForm(f => ({ ...f, establishedOn: e.target.value }))}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
                  </div>
                </>
              )}
              <div>
                ...
                <select value={form.closingMonth} onChange={e => setForm(f => ({ ...f, closingMonth: e.target.value }))}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm">
