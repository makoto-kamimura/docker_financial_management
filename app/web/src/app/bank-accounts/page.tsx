...
type BankAccount = {
  ...
  accountType: string; accountNumber: string | null; balance: number;
  _count: { transactions: number };
};
type Tx = {
  id: number; date: string; description: string;
  amount: number; balance: number | null; category: string | null;
};
type FlowTransferRow = {
  id: number; from: string | null; to: string | null;
  amount: number; kind: string; channel: string; channelLabel: string;
  label: string | null; day: number; note: string | null;
};
    ...
    <svg viewBox={`0 0 ${SVG_W} ${svgHeight}`} className="w-full" style={{ height: svgHeight }}>
      <defs>
        {strokeKeys.map(ch => {
          ...
          return (
            <marker key={ch} id={`arr-${ch}`} markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto">
      ...
      {(["収入元", "口座", "支出先"] as const).map((label, col) => (
        <text key={col} x={COL_X[col] + NODE_W / 2} y={22}
          textAnchor="middle" fontSize={10} fill="#94a3b8"
          fontFamily="system-ui, sans-serif" fontWeight={700} letterSpacing="0.08em">
          {label.toUpperCase()}
        </text>
      ))}
      ...
      {edges.map((e, i) => {
        ...
        return (
          <g key={i}>
            ...
            />
            {/* Label pill */}
            <rect x={midX - 62} y={midY - 19} width={124} height={38} rx={6}
              fill="white" stroke="#e2e8f0" strokeWidth={1} filter="url(#card-shadow)" />
            <text x={midX} y={midY - 5} textAnchor="middle"
              fontSize={12} fill="#1e293b" fontFamily="system-ui, sans-serif" fontWeight={700}>
      ...
      {[...nodeMap.entries()].map(([id, n]) => {
        ...
        const lines = paren > 0
          ? [clip(id.slice(0, paren), 10), clip(id.slice(paren), 10)]
          : [clip(id, 13)];

        return (
          <g key={id}>
            <rect x={n.x} y={n.y} width={NODE_W} height={NODE_H} rx={10}
              fill={fill} stroke={stroke} strokeWidth={1.5} filter="url(#card-shadow)" />
            {lines.length === 1 ? (
              <text x={n.x + NODE_W / 2} y={n.y + NODE_H / 2}
                textAnchor="middle" dominantBaseline="middle"
                fontSize={12.5} fill={textColor} fontFamily="system-ui, sans-serif" fontWeight={600}>
                {lines[0]}
              </text>
            ) : (
              <>
                <text x={n.x + NODE_W / 2} y={n.y + NODE_H / 2 - 8}
                  textAnchor="middle" dominantBaseline="middle"
                  fontSize={12} fill={textColor} fontFamily="system-ui, sans-serif" fontWeight={600}>
                  {lines[0]}
                </text>
                <text x={n.x + NODE_W / 2} y={n.y + NODE_H / 2 + 9}
                  textAnchor="middle" dominantBaseline="middle"
                  fontSize={10} fill={textColor} fontFamily="system-ui, sans-serif">
                  {lines[1]}
                </text>
              </>
            )}
...
export default function BankAccountsPage() {
  ...
  const { data: flow } = useQuery({
    ...
    queryFn: async (): Promise<FlowResponse> =>
      (await fetch("/api/transfers/flow")).json(),
  });
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {isLoading ? (
          ...
          <button key={a.id} onClick={() => setSelected(a)}
            ...
            <div className="text-lg font-bold text-indigo-600 mt-1">¥{(a.balance ?? 0).toLocaleString()}</div>
              ...
              <tbody className="divide-y divide-slate-100">
                {txs.length === 0 ? (
                  ...
                  <tr key={tx.id} className="hover:bg-slate-50">
                    ...
                    <td className={`px-4 py-2.5 font-medium ${tx.amount >= 0 ? "text-emerald-600" : "text-red-600"}`}>
                      {tx.amount >= 0 ? "+" : ""}¥{tx.amount.toLocaleString()}
                    </td>
                    <td className="px-4 py-2.5 text-slate-400">{tx.balance != null ? `¥${tx.balance.toLocaleString()}` : "—"}</td>
