...
export default function FiscalYearsPage() {
  ...
  const loadAll = () => {
    ...
    Promise.all([
      ...
      fetch(`/api/fiscal-years${selectedTenant ? "?tenantId=" + selectedTenant : ""}`).then((r) =>
        r.json(),
      ),
    ]).then(([t, fy]) => {
      setTenants(t.data ?? []);
                ...
                fiscalYears.map((fy) => (
                  <tr key={fy.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium ${fy.status === "open" ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-600"}`}
                      >
