...
export default function LedgerPage() {
  ...
  const search = () => {
    ...
    fetch(`/api/reports/general-ledger${qs}`)
      .then((r) => r.json())
      .then((j) => {
        setLedgers(j.data ?? []);
        setLoading(false);
    ...
    <AppShell>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 mb-6 flex gap-4 items-end">
        ...
        <div>
          ...
          <select
            value={accountId}
            onChange={(e) => setAccId(e.target.value === "" ? "" : Number(e.target.value))}
          ...
          >
            <div className="px-5 py-3 bg-slate-50 border-b border-slate-200 flex justify-between">
              ...
              <div className="text-sm text-slate-500 space-x-4">
                <span>
                  借方合計:{" "}
                  <strong className="text-blue-700">¥{l.totalDebit.toLocaleString()}</strong>
                </span>
                <span>
                  貸方合計:{" "}
                  <strong className="text-red-700">¥{l.totalCredit.toLocaleString()}</strong>
                </span>
                <span>
                  残高:{" "}
                  <strong
                    className={
                      l.totalDebit - l.totalCredit >= 0 ? "text-green-700" : "text-red-700"
                    }
                  >
              ...
              <tbody className="divide-y divide-slate-50">
                {l.rows.map((row, i) => (
                  <tr key={i} className="hover:bg-slate-50">
                    ...
                    <td
                      className={`px-4 py-2 font-medium ${row.balance >= 0 ? "" : "text-red-600"}`}
                    >
